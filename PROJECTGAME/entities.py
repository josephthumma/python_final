...
def objectcollide(object, other_object):
...
class SpaceShip:
    ...
    def shoot(self):
        if self.cool_down_counter == 0:
            laser= Blast(self.x + int(self.rocket.get_width()/2), self.y, self.rocket)
...
class SpaceEnemy(SpaceShip):
    TYPE_MAP = {
        "gamesoldier" : (ENEMY_UFO, ENEMY_BULLET, UFO_HEALTH, UFO_POINTS),
        "gamecaptain" : (ENEMY_UFO_STRONG, ENEMY_BULLET, UFO_STRONGER_HEALTH, UFO_STRONGER_POINTS),
        "gameboss" : (ENEMY_UFO_BOSS, ENEMY_BULLET, BOSS_HEALTH, UFO_BOSS_POINTS)
    }
